...
export default function PropertyForm() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setCompressing(true);
    toast.info('Otimizando e comprimindo imagens no navegador...');
    ...
    try {
      ...
      toast.success(`${compressedUrls.length} imagem(ns) otimizada(s) com sucesso!`);
    } catch (err) {
      console.error(err);
  ...
  const addImage = async () => {
    if (newImageUrl.trim()) {
      try {
        ...
        setNewImageUrl('');
        toast.success('Imagem adicionada e otimizada!');
      } catch {
        setFormData((prev) => ({
          ...prev,
            ...
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  ⚡ Otimização automática: as fotos são comprimidas em WebP (máx 1920px) no seu navegador para economizar espaço no Supabase.
                </p>
              </div>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex flex-col sm:flex-row gap-3">
                    ...
                    >
                      {compressing ? (
                        ...
                        <>
                          ...
                          Enviar Fotos do Computador/Celular
                        </>
                      )}
                    </Button>
                  </label>
                ...
                <div className="flex flex-1 gap-2">
                  <Input
                    ...
                    disabled={compressing}
                  />
                  <Button type="button" variant="outline" onClick={addImage} disabled={compressing}>
                ...
                {formData.images?.map((url, index) => (
                  <div key={index} className="relative group rounded-lg overflow-hidden border bg-muted">
                    ...
                    <button
                      ...
                      className="absolute top-2 right-2 bg-destructive text-destructive-foreground p-1.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity shadow-md"
                    >
                      <X size={16} />
                    </button>
                    {index === 0 && (
                      <div className="absolute bottom-2 left-2 bg-primary text-primary-foreground px-2 py-0.5 rounded text-xs font-semibold shadow">
                        Capa
                      </div>
                    )}
                  </div>
